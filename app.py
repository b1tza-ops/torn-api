#!/usr/bin/env python3
"""Torn Deal Finder Pro v4.1 — read-only market intelligence + Telegram control."""
...
def api(path, key, params=None, retries=4):
    headers={"Authorization":f"ApiKey {key}","Accept":"application/json","User-Agent":"TornDealFinderPro/4.1"}
    delay=3
...
def market(item_id,key,limit=100):
    ...
    raw=find_list(payload,("itemmarket","item_market","listings")); avg=None
...
def set_setting(c,k,value):
    c.execute("INSERT OR REPLACE INTO settings(k,v) VALUES(?,?)",(k,json.dumps(value))); c.commit()
...
def turnover_score(c,item_id,hours=6):
    cut=now()-int(hours*3600); active_cut=now()-900
    total=c.execute("SELECT COUNT(*) FROM listing_state WHERE item_id=? AND first_seen>=?",(item_id,cut)).fetchone()[0]
    gone=c.execute("SELECT COUNT(*) FROM listing_state WHERE item_id=? AND first_seen>=? AND last_seen<?",(item_id,cut,active_cut)).fetchone()[0]
...
def reference(listings,api_avg,hist,cfg):
    ...
    band=float(cfg.get("support_band_percent",7)); support=sum(1 for p in prices if abs(p-m)/m*100<=band)
    mean=sum(sample)/len(sample); cv=(sum((p-mean)**2 for p in sample)/len(sample))**.5/mean if mean else 1
...
def build_watch(cat,cfg,c):
    s=cfg.get("scanner",{}); cats=[str(x).lower() for x in s.get("categories",[])]; names={str(x).lower() for x in s.get("item_names",[])}; ids={int(x) for x in s.get("item_ids",[])}
    adds=set(get_setting(c,"watch_add",[])); removes=set(get_setting(c,"watch_remove",[])); out=[]
...
def evaluate(item,listings,ref,conf,support,turnover,cfg,c):
    r=rules_for(item,cfg,c); fee=float(r.get("sale_fee_percent",cfg.get("sale_fee_percent",5)))/100
    min_roi=float(r.get("min_roi_percent",7)); min_each=float(r.get("min_profit_per_item",3000)); min_total=float(r.get("min_total_profit",15000)); min_disc=float(r.get("min_discount_percent",7)); min_conf=float(r.get("min_confidence_percent",45)); min_support=int(r.get("min_support_listings",cfg.get("min_support_listings",5)))
    maxcost=float(r.get("max_total_cost",cfg.get("bankroll",{}).get("max_capital_per_deal",5000000))); maxqty=int(r.get("max_quantity_to_value",1000))
...
def catalog_page(cat,arg=""):
    ...
    title=f"📦 Items{(' — '+category) if category else ''} ({len(items)})\nPage {page}/{pages}"
    body="\n".join(f"• {x['name']} [{x['type']}] — ID {x['id']}" for x in chunk)
    hint="\nUse /items <category> <page> or /find <name>"
    return title+"\n"+(body or "No matching items.")+hint
...
def find_items_text(cat,q):
    ...
    if not rows:return "No matching items."
    return "🔎 Matches\n"+"\n".join(f"• {x['name']} [{x['type']}] — ID {x['id']}" for x in rows)


def cmd_help():
    return ("🤖 Torn Deal Finder Pro v4.1\n"
            "/status — scanner state\n/top — best recent deals\n/history <item> — price history\n"
            "/items [category] [page] — browse Torn items\n/categories — item categories\n/find <text> — search items\n/item <name> — live market snapshot\n"
            "/watch <item> — add item\n/unwatch <item> — remove item\n/watchlist — current watchlist\n"
            "/budget <amount> — max capital/deal\n/minprofit <amount> — min total profit\n/minroi <percent> — min ROI\n"
    ...
    for u in updates:
        ...
        try:
            if cmd in ("/start","/help"):tg(token,chat,cmd_help())
                ...
                ref,conf,support,reason=reference(ls,avg,hist,cfg) if ls else (None,0,0,"no listings")
                lines=[f"📦 {item['name']} [{item['type']}]",f"ID: {item['id']}",f"Listings: {len(ls)}"]
...
def main():
    ...
    offset=int(get_setting(c,"telegram_offset",0) or 0);tg(token,chat,f"✅ Torn Deal Finder Pro v4.1 started\nWatching {len(items)} items\nCatalog: {len(cat)} items\nUse /help for commands")
    cycle=0
    while not STOP:
